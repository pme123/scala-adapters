...
(implicit val mat: Materializer, val ec: ExecutionContext)
  ...
  protected def runAdapter(user: String): Future[Any] = {
    val log = LogService("Test Common Adapter Process", user, Some(self))
    logService = Some(log)
    Future {
      log.startLogging()
      for (i <- 0 to 10) {
